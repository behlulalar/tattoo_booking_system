...
sudo systemctl status roof-tattoo-backend --no-pager -l | head -15
...
sudo journalctl -u roof-tattoo-backend -n 30 --no-pager
...
sudo journalctl -u roof-tattoo-backend --since "5 minutes ago" | grep -i "database\|veritabanı\|connection" | tail -10
...
if [ -f "/opt/roof_tattoo/backend/wapio_settings.json" ]; then
    echo "✅ wapio_settings.json bulundu:"
    cat /opt/roof_tattoo/backend/wapio_settings.json | python3 -m json.tool 2>/dev/null || cat /opt/roof_tattoo/backend/wapio_settings.json
else
...
echo "[8] .env dosyası:"
if [ -f "/opt/roof_tattoo/backend/.env" ]; then
    echo "✅ .env dosyası var"
    echo "   DATABASE_HOST: $(grep DATABASE_HOST /opt/roof_tattoo/backend/.env | cut -d'=' -f2 | head -c 30)..."
    echo "   WAPIO_INSTANCE_ID: $(grep WAPIO_INSTANCE_ID /opt/roof_tattoo/backend/.env | cut -d'=' -f2 | head -c 30)..."
else
